-- Master Fix Script (v2 - Fixed Syntax & Ambiguities)
-- Date: 2025-12-20
...
-- 2. Function Definitions (with prefix 'p_' for parameters to avoid ambiguity)
...
-- Overload: remove_follower(p_target_user_id) - assumed to mean "I want to stop following target" OR "Remove target from my followers"?
-- Based on error 42883 "function remove_follower() does not exist" in previous context which likely had NO args or 1 arg.
-- If the user wants "remove_follower(target_id)" usually implies "remove this person from my followers list" OR "unfollow this person".
-- Given 'unfollow_user' exists below, let's assume this overload facilitates the "unfollow" action or "remove follower" action.
-- Let's define it as "Unfollow" for consistency with `unfollow_user`.
CREATE OR REPLACE FUNCTION public.remove_follower(p_target_user_id UUID)
RETURNS VOID AS $$
BEGIN
    -- Assumes 'unfollow' semantics: Auth user stops following p_target_user_id
...
EXCEPTION WHEN OTHERS THEN 
    NULL; -- Ignore errors if functions missing/mismatched during batch alter
END $$;
